...
export function EditPeople({ people, onAddPerson, onRemovePerson, onRenamePerson, onSetPersonColor, onDone }: EditPeopleProps) {
  ...
  return (
    <div className="flex-1 flex items-start justify-center overflow-y-auto" style={{ paddingTop: 90, paddingBottom: 40 }}>
      <div className="bg-white" style={{ width: 720, maxWidth: "90vw", borderRadius: 22, boxShadow: "0 1px 3px rgba(0,0,0,.06)", padding: "40px 44px" }} data-testid="edit-people-card">
        <h2 style={{ margin: "0 0 6px", fontSize: 34, fontWeight: 800, letterSpacing: "-.5px", color: "#2b3038" }}>
          Set up your chore crew
        </h2>
        <p style={{ margin: "0 0 28px", fontSize: 18, fontWeight: 500, color: "#9aa0aa" }}>
              ...
              <div key={p.id} className="flex flex-col gap-3" style={{ background: pal.tint, borderRadius: 14, padding: "14px 16px" }} data-testid={`setup-person-${p.id}`}>
                <div className="flex items-center gap-3.5">
                  ...
                  {isEditing ? (
                    <>
                      <input
                        ...
                        style={{ height: 46, border: `2px solid ${pal.color}`, borderRadius: 12, padding: "0 14px", fontSize: 19, fontWeight: 700, color: "#2b3038" }}
                    ...
                    <button
                      ...
                      onClick={() => startEdit(p)}
                      title="Edit name"
                      className="flex-1 min-w-0 flex items-center gap-2 text-left rounded-[10px] hover:bg-white/55"
                  ...
                  <button
                    ...
                    title="Remove"
                    className="touch-button flex-shrink-0 flex items-center justify-center rounded-full hover:bg-white"
                    style={{ width: 40, height: 40, border: "none", background: "rgba(255,255,255,.65)", color: pal.text }}
        ...
        <div className="flex gap-2.5" style={{ marginBottom: 30 }}>
          ...
          <button
            type="button"
            onClick={addPerson}
            className="touch-button flex-shrink-0 flex items-center justify-center gap-2 text-white"
            style={{ height: 56, padding: "0 24px", border: "none", background: "#2b3038", borderRadius: 14, fontSize: 18, fontWeight: 700 }}
        ...
        <button
          ...
          style={{ height: 62, border: "none", background: "var(--rb-accent)", borderRadius: 999, fontSize: 21, fontWeight: 800, boxShadow: "0 2px 6px rgba(242,101,90,.35)" }}
