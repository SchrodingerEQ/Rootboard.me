...
- [ ] **Step 5: Verify build still clean:** `npm run build` → exits 0.
...
- [ ] **Step 1: Create `client/src/lib/app-types.ts`**
...
- [ ] **Step 3: Point everything at `@/lib/app-types`** — delete the `Section` declaration in `nav-rail.tsx:4` and the `CalendarView` declaration in `calendar.tsx`, add `import type { Section } from "@/lib/app-types"` / `import type { CalendarView, Section } from "@/lib/app-types"` respectively, and change `calendar-header.tsx:7` to `import type { CalendarView } from "@/lib/app-types";`. Keep `export type { Section }` in `nav-rail.tsx` and `export type { CalendarView }` in `calendar.tsx` ONLY if the grep in Step 2 showed other importers you are not updating — otherwise remove the re-exports.

- [ ] **Step 4: Verify:** `npm run check` exits 0; `git grep -n 'from "@/pages/calendar"' client/src/components` → no output.

- [ ] **Step 5: Commit** — `refactor: move Section/CalendarView types to lib/app-types`
...
- [ ] **Step 1: Confirm it is dead:** `git grep -nE '\bRB\b' client/src` → only the definition in `color-utils.ts` (audit 2026-08-19 found no other references; re-verify).
- [ ] **Step 2: Delete lines 26–40** (the `// Shared Rootboard UI tokens…` comment + `export const RB = {…} as const;`). The three functions above it stay.
- [ ] **Step 3: Verify:** `npm run check` and `npm test` pass.
- [ ] **Step 4: Commit** — `refactor: remove unused RB color constant (duplicated --rb-* palette)`
...
- [ ] **Step 1: Pass the filtered list to MonthView** — in `calendar.tsx:382-388`, change `events={events}` to `events={filteredEvents}` and delete the `enabledCalendars={enabledCalendars}` prop.
- [ ] **Step 2: Strip MonthView's internal filter** — in `month-view.tsx`: remove `enabledCalendars?: Set<string>;` from the props interface (line 12) and the destructure (line 19); delete `const usingFilter = …` (line 50) and the `if (usingFilter && …) continue;` line (line 53); remove `enabledCalendars` from the `eventsByDate` dependency array (line 77).
- [ ] **Step 3: Remove the now-dead prop from Week/Day** — grep inside `week-view.tsx` and `day-view.tsx` for `enabledCalendars`; if (as expected) it is only received and never used, delete it from their prop interfaces and from the call sites at `calendar.tsx:398` and `:411`. If either view *does* use it, leave that view untouched and note it in the completion report.
- [ ] **Step 4: Verify:** `npm run check`, `npm test`, `npm run build` pass. Manual check in the dev app: month/week/day render identically with a normal calendar selection; toggling all calendars off now empties all three views.
- [ ] **Step 5: Update `docs/SPEC.md`** — rewrite the §3.2 note ("Inconsistency to know about: …") to say all views receive pre-filtered events, and note the empty-set unification.
- [ ] **Step 6: Commit** — `refactor: single pre-filtered events path for all calendar views`
...
- [ ] **Step 2: Build the table** in `phase2-color-inventory.md`: columns `file · line · literal · role · variable (new or existing)`. Naming rules (from [phase0-color-sweep-prompt.md](../theme-system/phase0-color-sweep-prompt.md)):
...
- [ ] **Step 3: Commit the table** (`refactor: phase 2 color inventory + naming table`) and **STOP — present the table (especially new variable names and all `?` rows) for founder approval before Task 7.** Tasks 1–4 and 6 may proceed meanwhile.
...
- [ ] **Step 1: Extend the `--rb-*` block in `index.css`** with every *approved* new variable from Task 5. The pre-decided set (final values confirmed against the inventory):
...
- [ ] **Step 2: Add the `rb` color map to `tailwind.config.ts`** inside `theme.extend.colors` (enables `bg-rb-surface`, `text-rb-muted`, … for the palette-class sweep):
...
- [ ] **Step 3: Verify:** `npm run build` passes; app renders identically (no consumer changed yet).
- [ ] **Step 4: Commit** — `refactor: add phase-2 --rb-* tokens and Tailwind rb color map`
...
- [ ] **Step 1: `nav-rail.tsx` replacements** (exact edits):
...
- [ ] **Step 2: `index.css` consumer sites:** `.calendar-cell` shadow → `box-shadow: 0 1px 2px var(--rb-shadow-soft);`; `.screensaver-overlay` gradient → `linear-gradient(135deg, var(--rb-screensaver-bg-1) 0%, var(--rb-screensaver-bg-2) 50%, var(--rb-screensaver-bg-1) 100%)`; `scrollbar-color: var(--rb-scrollbar-thumb) transparent;`; both `::-webkit-scrollbar-thumb` backgrounds → `var(--rb-scrollbar-thumb)` / `var(--rb-scrollbar-thumb-hover)`. Do NOT restructure the `@supports` guard (see the war-story comment — it is load-bearing).
- [ ] **Step 3: Gate:** `git grep -nE '#[0-9a-fA-F]{3,8}\b|rgba?\(' client/src/components/nav-rail.tsx` → no output. `npm run build && npm test` pass. Visual check: rail, badge, scrollbars, screensaver dim identical.
- [ ] **Step 4: Commit** — `refactor: color variables for nav rail and global css`
...
- [ ] **Step 2: Gate:** the Task 5 hex/rgb/palette greps restricted to `client/src/components/calendar client/src/pages/calendar.tsx` return only allowlisted lines (`calendar-meta.ts` `FALLBACK_COLORS` + `EVENT_FALLBACK_COLOR` definition). `npm run build && npm test` pass. Visual check all three views + settings popover + each dialog.
- [ ] **Step 3: Commit(s)** — `refactor: color variables for calendar views/header`, `refactor: color variables for settings and dialogs`
...
- [ ] **Step 1: Write the failing spec** (`confetti-colors.spec.ts`):
...
- [ ] **Step 2: Run it — must fail:** `npx vitest run client/src/components/chores/confetti-colors.spec.ts` → FAIL ("Expected 0 arguments, but got 1" / injection test fails).
- [ ] **Step 3: Make `generateConfettiParticles` injectable** (`confetti-burst.tsx:23`, one-line signature change; body's line 32 uses the param):
...
- [ ] **Step 4: Resolve runtime colors in the caller** — in `chore-card-stack.tsx`, add and use:
...
- [ ] **Step 5: Run tests:** `npm test` → legacy chores/dinner scripts still pass (pure functions untouched), new spec passes.
- [ ] **Step 6:** Apply the Task 5 table to the remaining chores files (straight literal→`var()`/`rb`-utility swaps).
- [ ] **Step 7: Gate:** area greps return only `PERSON_PALETTE` + `CONFETTI_COLORS` lines; build + tests pass; visual check incl. a confetti burst and the FLIP reorder.
- [ ] **Step 8: Commit** — `refactor: color variables for chores; theme-injectable confetti colors`
...
- [ ] **Step 1:** Apply the approved table (straight swaps; `lib/dinner-state.ts` has no styling colors — verify with the grep, and leave its logic untouched).
- [ ] **Step 2: Gate:** area greps clean; `npm run build && npm test` pass (dinner legacy script pins the pure logic). Visual check: voting strip, planner grid, both dialogs.
- [ ] **Step 3: Commit** — `refactor: color variables for dinner`
...
- [ ] **Step 1:** Apply the table to each remaining inventory row.
- [ ] **Step 2: Gate:** full-repo greps (Task 5 commands) return ONLY allowlisted lines. `npm test` (OSK legacy script pins the keyboard core) and `npm run build` pass. Visual check: open the OSK on a touch field — solid background, dialogs lift, emoji layer renders.
- [ ] **Step 3: Commit** — `refactor: color variables for keyboard, screensaver, and remaining files`
